...
typedef enum
{
    ...
    // stream image timestamp: wparam: major image pts(ms), lparam: minor image pts(ms)
    E_SV_METADATA_TYPE_IMAGE_TIMESTAMP                   = 8,
    // push flow command reqest timestamp: wparam: 0, lparam: timestamp ptr(int64_t*)
    E_SV_METADATA_TYPE_PUSHFLOW_REQUEST_TIMESTAMP         = 9
 ...
 * @param       auth_info : Sunvalley to b auth info
 ...
 ***************************************************************************************************************/
long SVPush_API_Connect_By_DeviceSN(struct auth_info* pai, const char* pdeviceSN, event_callback pcbfun);
...
/**
 * @describe record send packet to local file
 * @param1 lcid : Connected ID of push handle
 * @param  precord_url : record file save url
 * @return Success return 0, else return < 0
 * @remark this function is use for debug
 */
