...
#include "sv_log.h"
//#define ENALBE_MEDIA_PARSER
#ifdef ENALBE_MEDIA_PARSER
...
public:
	...
	{
        lbtrace("vps:%p, sps:%p, pps:%p, aac_cfg:%p\n", vps.data(), sps.data(), pps.data(), aac_cfg.data());
		return -1;
	}
};
...
{
public:
    virtual ~IRecordMuxer(){}

    virtual int open(const char* purl, codec_id vcodecid, codec_id acodecid,  int encflag, int64_t mediatimestamp = -1) = 0;

    virtual int writeframe(codec_id codecid, char* pdata, int len, int keyflag, int64_t pts) = 0;

    virtual void close() = 0;
};


class RecordMuxer : public IRecordMuxer
{
public:
    RecordMuxer()
    {
    ...
    int writeframe(codec_id codecid, char* pdata, int len, int keyflag, int64_t pts)
    {
        if(!m_pfile)
        {
        ...
        m_recframe.type = codecid == 8 ? codecid:keyflag;
...
class HS004RecordMuxer : public IRecordMuxer
{
protected:
    FILE*                   m_pfile;
    VAVA_HS004_RecInfo      m_RecHdr;
    VAVA_HS004_RecHead      m_RecPkt;
public:
    HS004RecordMuxer()
    {
        m_pfile = NULL;
        memset(&m_RecHdr, 0, sizeof(m_RecHdr));
        memset(&m_RecPkt, 0, sizeof(m_RecPkt));
    }
    ...
    virtual int open(const char* purl, codec_id vcodecid, codec_id acodecid,  int encflag, int64_t mediatimestamp = -1)
    {
        if(NULL == purl)
        {
            lberror("Invalid parameter, purl:%s\n", purl);
            return -1;
        }
        memset(&m_RecHdr, 0, sizeof(m_RecHdr));
        m_RecHdr.tag = 0;
        m_RecHdr.ver = 1;
        m_RecHdr.v_encode  = (char)vcodecid;
        m_RecHdr.a_encode  = (char)acodecid;
        m_RecHdr.res  = 10;
        m_RecHdr.fps  = 15;
        m_RecHdr.encrypt = encflag;
        m_RecHdr.alarmtype = 6;
        m_RecHdr.sample = 16000;
        m_RecPkt.framnum = -1;
        m_pfile = fopen(purl, "wb");
        if(NULL == m_pfile)
        {
            lberror("create file %s failed, reason:%s\n", purl, strerror(errno));
            return -1;
        }
        fseek(m_pfile, sizeof(m_RecHdr), SEEK_SET);
        return 0;
    }
    /*  unsigned int tag;        //同步头 0xEB0000AA
  unsigned int size;        //帧大小
  ...
  unsigned int reserv;      //保留*/
    virtual int writeframe(codec_id codecid, char* pdata, int len, int keyflag, int64_t pts)
    {
        m_RecPkt.tag = 0xEB0000AA;
        m_RecPkt.size = len;
        m_RecPkt.type = 3 == codecid ? 8 : keyflag;
        m_RecPkt.ntsamp = pts;
        m_RecPkt.fps = 15;

        m_RecPkt.framnum++;
        m_RecHdr.size += len;
        m_RecHdr.time = m_RecHdr.time < pts ? pts : m_RecHdr.time;
        size_t write_len = fwrite(&m_RecPkt, 1, sizeof(m_RecPkt), m_pfile);
        if(write_len < sizeof(m_RecPkt))
        {
            lberror("write_len:%d = fwrite((&m_RecPkt, 1, sizeof(m_RecPkt):%d, m_pfile:%p)\n", write_len, sizeof(m_RecPkt), m_pfile);
            return -1;
        }

        write_len = fwrite(pdata, 1, len, m_pfile);
        if(write_len < (size_t)len)
        {
            lberror("write_len:%d = fwrite(pdata:%p, 1, len:%d, m_pfile:%p)\n", write_len, pdata, len, m_pfile);
            return -1;
        }
    ...
    virtual void close()
    {
        ...
        {
            m_RecHdr.tag = 1;
            m_RecHdr.vframe = m_RecPkt.framnum;
            m_RecHdr.time = m_RecHdr.time/1000;
            //lbtrace("");
            fseek(m_pfile, 0, SEEK_SET);
            fwrite(&m_RecHdr, 1, sizeof(m_RecHdr), m_pfile);
            fclose(m_pfile);
            m_pfile = NULL;
        }
    }
